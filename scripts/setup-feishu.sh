...
CC_BRIDGE_CONFIG="${HOME}/.cc-bridge/config.toml"

echo "=== cc-bridge 飞书集成配置 ==="
echo ""
...
if grep -q 'name = "bridge"' "$CC_CONNECT_CONFIG" 2>/dev/null; then
  ...
  # 获取 cc-bridge 命令路径
  CC_BRIDGE_BIN=$(which cc-bridge 2>/dev/null || echo "")
  if [ -z "$CC_BRIDGE_BIN" ]; then
    # 尝试从项目目录查找
    SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
    PROJECT_DIR="$(dirname "$SCRIPT_DIR")"
    if [ -f "$PROJECT_DIR/dist/cc-bridge" ]; then
      CC_BRIDGE_BIN="$PROJECT_DIR/dist/cc-bridge"
      echo "⚠️  cc-bridge 未在 PATH 中，使用编译产物: $CC_BRIDGE_BIN"
    else
      echo "❌ 未找到 cc-bridge 命令"
      echo "   请先安装 cc-bridge:"
  ...
  echo "✅ cc-bridge 路径: $CC_BRIDGE_BIN"
  ...
  # cc-bridge 会自动从 cc-connect session 文件中识别调用者身份
...
[[commands]]
  ...
  exec = "${CC_BRIDGE_BIN} feishu-cmd {{args}}"
EOF

  echo "✅ 已添加 bridge 命令配置"
fi

# 同步 bridge token 到 cc-bridge 配置
echo ""
...
if [ -n "$BRIDGE_TOKEN" ]; then
  # 确保 cc-bridge 配置目录存在
  mkdir -p "$(dirname "$CC_BRIDGE_CONFIG")"

  # 创建或更新 cc-bridge 配置
  if [ -f "$CC_BRIDGE_CONFIG" ]; then
    # 更新现有配置
    if grep -q '\[bridge\]' "$CC_BRIDGE_CONFIG"; then
      # 更新 token
      sed -i.bak "s/token = .*/token = \"${BRIDGE_TOKEN}\"/" "$CC_BRIDGE_CONFIG"
    else
      # 添加 bridge 段
      cat >> "$CC_BRIDGE_CONFIG" << EOF
...
EOF
  ...
  echo "✅ 已同步 Bridge API Token 到 $CC_BRIDGE_CONFIG"
else
  echo "⚠️  未在 cc-connect 配置中找到 bridge token"
  echo "   请手动配置: $CC_BRIDGE_CONFIG"
  echo "   添加以下内容:"
  echo '   [bridge]'
...
echo ""
echo "注意：cc-bridge 会自动从 cc-connect session 文件中识别飞书调用者身份"
