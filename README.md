...
cc-connect 的每个会话都有一个 `agent_session_id` 字段（UUID），它**直接对应 Claude Code 原生的会话 ID**。对应的 JSONL 文件存在于 `~/.claude/projects/<项目目录>/<agent_session_id>.jsonl`，包含完整的对话数据（用户消息、助手回复、thinking block、tool_use/tool_result、完整的 parentUuid 链）。
...
| `--sort <field>` | `-s` | 排序字段：last_active（默认）/ created_at / message_count |
...
4. **若 origin=cc-connect，校验是否仍是当前 active_session**（`reset_on_idle` 检测）
...
### 9.3 cc-connect 的 reset_on_idle_mins
