...
- **25KB 安全降级**：列表卡片超过 25KB 时自动退化为简洁文本消息，避免飞书 API 报错。Attached card（Attach 后的自动刷新卡）则采用 **smart truncation**：按 `[2048, 1024, 512, 256]` 字节预算逐级截断各字段，最后才显示 `⚠️ 内容过大` 提示
...
- **v2.4 Rendezvous 路径**（`rendezvous_enabled=true` 默认开启）：
  - `checkRendezvousEligibility` 决策树：state.json 存在 → bg 处于 waiting（blocked+needs / running/working+needs / blocked）→ roster 有 `rendezvousSock` → socket 文件存在且是 socket → 走 rendezvous
  - **Phase 1**：`submitReplyOnly()` 向 rendezvous socket 提交 text（fire-and-forget，daemon ack 一次后 close — 真 daemon 协议，**不是**长连接流式）
  - **Phase 2**：`pollStateJsonStreaming()` 每 500ms 轮询 state.json 等待终态（done/stopped/blocked+needs/idle/error），最长 `rendezvous_timeout_ms`（默认 60s），期间通过 `onPoll` 回调 patch 飞书卡片实时更新进度
  - **eligible 失败**（daemon_down / bg_busy / no_rendezvous_sock）→ 自动降级到 v2.3.5 SDK 路径
  - **bg 处理完又问新问题**（`new_needs`）：rendezvous 返回 `bgAskedNewQuestion: true`，handleReply 重新 `set()` expectedReply，用户可直接在 chat 接着回，无需再点 [Reply]（v2.4.x 链式回复 UX）
...
- **取消**：点击 `[取消等待]` 按钮或发送 `/agents` 返回列表。`clear()` 做 CAS `current → null`，即使 CAS 未命中也会清理内存状态和定时器（v2.2.19 修复）
...
- **Smart Truncation**：attached card 超过 25KB 时不做文本 fallback，而是按 `[2048, 1024, 512, 256]` 字节预算逐级截断字段，最后才显示 `⚠️ 内容过大, 请点 [Peek] 查看完整`
...
3. **Reply 真正 busy 的 worker 不可用**：v2.4 rendezvous 已支持向 **waiting 状态**（blocked + needs / running/working + needs）的 bg worker 投递 reply（通过 rendezvous socket 提交 + state.json 轮询终态）。但 worker 正在**真正 processing**（tempo=active，无 needs）时仍无法投递，只能等 worker 进入 waiting 或完成状态。见 `checkRendezvousEligibility` 的决策树。
...
- ~~Reply 活 worker 的 daemon IPC 支持~~ — **v2.4 已通过 rendezvous 解决**：waiting 状态的 bg worker 走 rendezvous socket 提交 + state.json 轮询终态，不再依赖 SDK
