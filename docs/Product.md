...
cc-bridge 是一个轻量级本地桥接工具，解决 cc-connect 和 Claude Code CLI 之间的**会话发现壁垒**问题。

cc-connect 将 Claude Code 桥接到飞书、微信等 IM 平台，让用户可以远程发起和继续 AI 编程对话。但它和 Claude Code 原生 CLI 之间存在会话隔离——两端各自管理会话列表，互不可见。

cc-bridge 通过建立一个**统一的会话注册表（Session Registry）**，让两端会话互相可见、互相恢复。
...
因此，cc-bridge 不需要做任何数据格式转换——只需要做一个**发现 + 注册 + 引导**的薄层。
...
| 手机 → 电脑  | 在飞书讨论了需求方案，回工位想在终端实现   | `cc-bridge resume` 一键恢复飞书会话                       |
| 多人协作     | Tech Lead 想查看所有成员的会话进展 | `cc-bridge list` 集中查看                             |
...
- 当前机器上的 `cc-connect` session JSON 可由 `cc-bridge` 读写
...
| 组件                    | 形态                           | 职责                                                |
| --------------------- | ---------------------------- | ------------------------------------------------- |
| **Registry**          | `~/.cc-bridge/registry.json` | 统一的会话索引，包含所有来源的会话元数据                              |
| **CLI 工具**            | `cc-bridge` 命令               | 终端用户交互入口：list / resume / sync                     |
...
用户: /bridge switch <uuid或短前缀>
         │
         ▼
cc-bridge 从 registry 找到目标 UUID
         │
...
| `project_dir`             | string    | 否  | Claude Code 的 project 目录名（如 `-Users-wuyujun-Git-cc-bridge`），仅用于显示，不参与路径定位                                                                                                   |
...
- **独立备份目录**：备份文件存放在 `~/.cc-bridge/backups/` 目录
...
- 高风险：CLI 命令前的扫描与 Hook 启动注册可能同时写入；`cc-bridge watch`（如启用）与 Hook 可能同时写入
- 中风险：用户手动执行 `cc-bridge sync` 时与其他 CLI 命令冲突
- 低风险：多个 CLI 命令同时执行
...
def write_with_lock(data: dict, max_retries: int = 3):
    ...
    raise CCBridgeError('E007', '注册表被锁，等待超时')
```

**第三方进程禁止直写 registry**：

`registry.json` 由 cc-bridge 内部多个组件（Scanner / Hook / CLI）共享，统一使用 fcntl.flock（advisory lock）。请勿用第三方工具（脚本、文本编辑器等）直接修改 `registry.json`，否则可能：

- 读到不一致的中间状态（写入未完成）
- 写入冲突（advisory lock 对非 cc-bridge 进程无效）
- 触发自动备份将损坏状态轮转写入

如需批量操作 registry，请通过 `cc-bridge` 命令进行（list / sync / clean / show 都已封装锁逻辑）。如需脚本化集成，使用 `cc-bridge list --format json` 等公开输出。
...
共 3 个会话。使用 cc-bridge resume <Ref> 或完整 UUID 恢复会话。
```
...
### 4.3 `cc-bridge resume`

恢复指定会话到 Claude Code CLI。

```bash
cc-bridge resume [TARGET] [OPTIONS]
```
...
# cc-bridge 进程被 claude 替换，Claude Code 退出后用户回到原 shell
```

- 假设 `claude` 命令在 PATH 中，可通过配置项 `general.claude_bin` 指定自定义路径
- 用 `execvp` 而非 `subprocess.run`：cc-bridge 不会挂着等子进程，行为等同直接敲 `claude --resume`
...
操作:
  ...
  cc-bridge export b21d6d04   导出为 markdown
```
...
# 手动触发（测试用）
cc-bridge hook session-start --session-id <uuid>
```
...
You can remove it anytime with: cc-bridge hook uninstall
```

### 4.7b `cc-bridge register`（内部命令）

将单个会话注册到 Registry。主要由 Hook 脚本调用，不建议用户直接使用。

```bash
cc-bridge register <UUID> [OPTIONS]
```
...
### 4.8 `cc-bridge export`

导出会话为可读格式。

```bash
cc-bridge export <编号或UUID> [OPTIONS]
```
...
def export_session(session_id: str, output_path: Path, max_messages: int = None):
    ...
    if not jsonl_path:
        raise CCBridgeError('E002', 'JSONL 文件不存在')
...
### 4.9 `cc-bridge search`

搜索会话。

```bash
cc-bridge search <query> [OPTIONS]
```
...
### 4.10 `cc-bridge clean`

清理 registry 中的无效记录。

```bash
cc-bridge clean [OPTIONS]
```
...
### 4.11 `cc-bridge feishu-cmd`

飞书侧 `/bridge` 命令的路由入口。cc-connect 通过 `exec = "cc-bridge feishu-cmd {{args}}"` 调用，负责解析参数并格式化输出。

```bash
cc-bridge feishu-cmd <subcommand> [args...]
```
...
2. 读取 ~/.cc-bridge/registry.json（共享锁）
3. 执行对应操作
...
因此，cc-bridge 通过 cc-connect 的 `[[commands]]` 机制注册 `/bridge` 命名空间的自定义命令，提供跨项目、跨用户的统一会话管理体验。
...
用户发送 `/bridge list` 时，cc-connect 执行 `cc-bridge feishu-cmd --caller <user> list`，cc-bridge 读取 registry.json 后格式化输出，返回给飞书用户。
...
cc-bridge 通过命令行参数接收调用者标识（由 cc-connect 在 `[[commands]]` 中注入）：
...
def filter_for_caller(sessions, caller):
    ...
    if caller is None:
        raise CCBridgeError('E019', '缺少调用者身份，拒绝返回飞书会话列表')
...
```
步骤 1: cc-bridge 读取 registry.json，找到目标 UUID
        → 验证 JSONL 文件存在
...
| 功能   | cc-connect 内置命令 | cc-bridge `/bridge` 命令    | 区别                                                                 |
| ---- | --------------- | ------------------------- | ------------------------------------------------------------------ |
...
cc-bridge **不强制后台 daemon**。Scanner 是"扫描行为"，通过三层触发覆盖不同场景：

| 触发点                       | 时机                                    | 范围               | 默认开启                             |
| ------------------------- | ------------------------------------- | ---------------- | -------------------------------- |
| **Hook**（快路径）             | Claude Code 启动时                       | 单个 UUID 即时注册     | ✅（执行 `cc-bridge hook install` 后） |
| **CLI 命令前**（默认）           | `cc-bridge list/resume/show/sync` 执行前 | 全量增量扫描（基于 mtime） | ✅                                |
| **`cc-bridge watch`**（可选） | filesystem watcher 实时检测               | 全量               | ❌（Phase 3）                       |

**实现要点**：

- 增量扫描结果缓存到 `~/.cc-bridge/scan_cache.json`（记录每个文件的 mtime）
...
class JSONLScanner:
    ...
    def __init__(self, registry: Registry, cc_connect_uuids: set[str] | None = None,
        ...
        # 文件路径 → 修改时间，跨次扫描复用（持久化到 ~/.cc-bridge/scan_cache.json）
...
**Hook 脚本行为** (`cc-bridge hook session-start`):
...
cc-bridge register "${SESSION_ID}" \
  --origin cli \
  --cwd "${PROJECT_DIR}" \
  --source terminal 2>> ~/.cc-bridge/hook.log
```
...
```bash
# 安装一个临时探针 hook
cat > /tmp/cc-bridge-probe.sh <<'EOF'
#!/bin/bash
{
  echo "=== $(date -Iseconds) ==="
  echo "PWD=$PWD"
  ...
  cat
  echo "=== end ==="
} >> /tmp/cc-bridge-probe.log 2>&1
EOF
chmod +x /tmp/cc-bridge-probe.sh

# 临时配置 ~/.claude/settings.json：
#   { "hooks": { "SessionStart": "/tmp/cc-bridge-probe.sh" } }
# 启动几个 Claude Code 会话，观察 /tmp/cc-bridge-probe.log
```
...
- **兜底扫描**：Hook 失败（任何关键字段缺失、registry 被锁等）时不报错退出，只记录到 `~/.cc-bridge/hook.log`；由 CLI 命令前的兜底扫描发现该会话（最多 1 次扫描的延迟）
...
> 主判别比 entrypoint 更可靠：cc-connect 的 session JSON 是"权威源"。即使 Claude Code 调整了 entrypoint 字段含义，也不影响 cc-bridge 的判别结果。
...
10:00  用户到工位，打开终端
       ...
       │
       ▼
10:01  cc-bridge 检测到 cwd=/Users/wuyujun 与当前目录不同
...
14:00  SessionStart hook 触发
       cc-bridge 注册会话 a3f8c1d2-... 到 registry
...
15:00  用户通勤路上，打开飞书
       ...
       │
       ▼
15:01  cc-bridge 发现该会话尚无 cc-connect 映射
       提示:
       "⚠️ 首次切换此 CLI 会话需要创建 cc-connect 映射，并重启/重载 cc-connect。
        当前对话会短暂中断，但会话历史会保留。继续？[Y/n]"
       │
       ▼
15:01  用户确认后，cc-bridge:
...
当前限制:
- cc-bridge 是本地工具，假设 cc-connect 和 CLI 在同一台机器
- JSONL 文件和 registry 都在本地

未来扩展:
- cc-bridge sync --remote <server-url> 通过 SSH/HTTP 拉取远端 registry
...
- 每个 agent_session_id 在 registry 中独立存在，list 列表平等显示
  （旧 UUID 不会被自动隐藏；用户可用 cc-bridge clean --older-than 30 清理）
...
- cc-bridge list 默认不显示 corrupted 会话
- cc-bridge list --archived 可查看
- 无法恢复，提示用户
```
...
- cc-bridge hook status 显示最后执行结果
- 日志输出到 ~/.cc-bridge/hook.log
...
处理:
- 启动时校验 version 字段
- 损坏时自动从最近备份恢复 (~/.cc-bridge/registry.json.bak)
  （保留最近 3 个备份版本）
- 无法恢复时提示用户执行 cc-bridge init 重建
```
...
cc-bridge 使用 `~/.cc-bridge/config.toml` 进行配置，支持多层覆盖：默认值 < 配置文件 < 环境变量。
...
# bridge API token（建议从环境变量读取：CC_BRIDGE_TOKEN）
token = ""
# API 请求超时（秒）
timeout = 30
...
| 环境变量                      | 配置项                     | 说明                                                            |
| ------------------------- | ----------------------- | ------------------------------------------------------------- |
| `CC_BRIDGE_CONFIG_PATH`   | —                       | 配置文件路径（默认 `~/.cc-bridge/config.toml`）                         |
...
| `CC_BRIDGE_CALLER_USER`   | —                       | 调用者标识（飞书调用时可由 cc-connect 通过 `--caller` 参数或本环境变量传入；CLI 调用时可不设） |
...
class Config:
    def __init__(self, config_path: Path = None):
        self.config_path = config_path or Path.home() / '.cc-bridge' / 'config.toml'
...
# Go
go install github.com/xxx/cc-bridge@latest

# 直接下载二进制
curl -L https://github.com/xxx/cc-bridge/releases/latest/download/cc-bridge-$(uname -s)-$(uname -m).tar.gz \
...
- **已有映射的场景**：调用 `POST /bridge/sessions/switch` 即可
  ...
  - 不影响其他用户的活跃会话
- **未映射的 CLI 会话**：
  1. `cc-bridge` 直接修改 `~/.cc-connect/sessions/*.json` 创建首次映射
...
cc-connect 支持通过 `[[commands]]` 配置段注册自定义斜杠命令。cc-bridge 的飞书命令通过此机制接入：
...
# 或使用 systemd (Linux)
cc-bridge service install
cc-bridge service start
...
| 错误码    | 说明                        | 解决方式                                                                   |
| ------ | ------------------------- | ---------------------------------------------------------------------- |
| `E001` | Registry 文件不存在            | 执行 `cc-bridge init`                                                    |
...
| `E011` | 文件权限不足                    | 检查文件权限：`ls -l ~/.cc-bridge/`，运行 `chmod 600 ~/.cc-bridge/registry.json` |
...
| `E015` | 备份文件损坏                    | 执行 `cc-bridge init` 重建 registry                                        |
| `E016` | 配置文件格式错误                  | 检查 `~/.cc-bridge/config.toml` 语法                                       |
| `E017` | 会话已归档                     | 使用 `cc-bridge list --archived` 查看                                      |
| `E018` | 会话已损坏                     | 无法恢复，标记为 corrupted                                                     |
| `E019` | 缺少调用者身份                   | 检查 cc-connect `[[commands]]` 是否注入 `--caller {{user}}`                  |
| `E020` | 版本不兼容                     | 升级 cc-bridge 或重建 registry                                              |
...
# 错误处理和用户提示
def handle_error(error: CCBridgeError):
    """根据错误码提供详细的诊断信息和修复建议"""
    suggestions = {
        'E001': ['运行 cc-bridge init 初始化 registry'],
        'E002': ['会话已被清理，无法恢复', '检查 Claude Code 是否清理了旧会话'],
        'E007': ['等待其他进程完成', '或手动删除 ~/.cc-bridge/registry.json.lock'],
        'E010': ['清理磁盘空间', '或移动 registry 到其他磁盘'],
        'E011': ['检查文件权限：ls -l ~/.cc-bridge/', '运行 chmod 600 ~/.cc-bridge/registry.json'],
    }
...
- `~/.cc-bridge/registry.json` 权限应为 `600`
- Hook 脚本权限应为 `755`
...
| 功能                       | 说明                             |
| ------------------------ | ------------------------------ |
| `cc-bridge init`         | 初始化 + 首次扫描                     |
| `cc-bridge list`         | 列出所有会话（稳定使用 UUID/短前缀，不使用跨命令编号） |
| `cc-bridge resume`       | 恢复会话到终端（自动处理 CWD 切换）           |
| `/bridge list`           | 飞书侧统一会话视图（要求 caller 注入）        |
| `/bridge resume`         | 飞书侧返回终端恢复命令                    |
| `/bridge switch`         | 飞书侧切换会话；未映射 CLI 会话支持首次创建映射     |
| `cc-bridge hook install` | 安装 SessionStart hook           |
| `cc-bridge sync`         | 手动同步                           |
| Registry 读写              | 含文件锁 + 备份                      |
...
| `cc-bridge export` | 导出为 markdown                                                     |
| `cc-bridge status` | 状态面板                                                             |
| `cc-bridge clean`  | 清理无效记录                                                           |
| `cc-bridge watch`  | 常驻进程                                                             |
...
|---|---|---|---|
| 1 | `cc-bridge show`（4.4 节）: 显示最近消息内容 | 已实现元数据展示，**不展示最近消息** | Phase 2 功能 |
| 2 | `cc-bridge search`（4.9 节）: `--in-content` 始终搜索 JSONL | `--in-content` 仅在标题/预览无结果时搜索 | Phase 2 功能 |
...
| # | 功能 / spec 位置 | 当前实现状态 | 原因 |
|---|---|---|---|
| 4 | `cc-bridge export`（4.8 节）: `writeStream.cork()/uncork()` 批处理 | 每 100 条 `write('')` 刷新缓冲区，未配对 `cork()` | Phase 3 将实现完整的 `cork()/uncork()` 批处理 |
| 5 | `cc-bridge status`（4.6 节）: 显示 CLI 版本号 + 真实 last sync 时间 | 显示文件 mtime，不显示版本号 | Phase 3 功能 |
| 6 | `cc-bridge clean`（4.10 节） | 已实现基本功能，但无集成测试 | Phase 3 功能 |
...
|---|---|---|---|
| 7 | `cc-bridge resume`（4.3 节）: 应使用 `execvp` 替换进程 | 使用 `Bun.spawnSync` 启动 `claude --resume` | `spawnSync` 功能正确、保留退出码、体验等同直接执行。Bun 运行时无原生 `execve` 系统调用，改造为进程替换需要 `syscall` 绑定或 Go 重写，收益低风险高。 |
...
| 维度     | cc-connect 原生   | Claude Code 原生 | cc-bridge                  |
| ------ | --------------- | -------------- | -------------------------- |
| 会话可见性  | 只看 cc-connect 的 | 只看本地的          | 两端都看                       |
| 恢复方式   | `/switch`       | `--resume`     | `cc-bridge resume`         |
...
**Q: cc-bridge 会修改我的对话数据吗？**

A: 不会修改 JSONL 对话内容。cc-bridge 会维护自己的 registry；在一期中，为了让飞书继续 CLI 会话，`/bridge switch` 在首次映射时会改写 cc-connect 的 session JSON 映射关系，但不会改写对话历史本身。

**Q: 如果我删除了 registry.json，会话会丢失吗？**

A: 不会。JSONL 文件仍在，执行 `cc-bridge init` 会重新扫描并重建 registry。

**Q: cc-bridge 支持远程 cc-connect 吗？**
...
A: 以 JSONL 为准。cc-bridge 的 resume 直接基于 JSONL 文件，不依赖 cc-connect 的简化历史。
...
├── hook.log              # Hook 执行日志
├── cc-bridge.log         # 主程序日志（可选）
└── config.toml           # 可选配置
```

### 16.4 cc-bridge 与 cc-connect 的 Bridge API
...
cc-bridge 的 `/bridge switch` 在当前文档中覆盖两条路径：
...
### 16.5 cc-bridge 与 cc-connect 的 Commands 机制

cc-connect 支持 `[[commands]]` 配置注册自定义斜杠命令，cc-bridge 的飞书侧命令通过此机制接入：
...
cc-connect 解析命令，匹配到 "bridge"
  │
  ▼
cc-connect 执行: cc-bridge feishu-cmd --caller <user> list
  │
  ▼
cc-bridge 读取 ~/.cc-bridge/registry.json
  │
  ▼
cc-bridge 格式化输出（Markdown 文本或飞书卡片）
  │
  ▼
