...
cc-bridge 是一个轻量级本地 CLI 工具，解决 cc-connect（飞书/微信等 IM 平台）和 Claude Code CLI 之间的**会话发现壁垒**问题。
...
│   │   │   ├── register.ts      # cc-bridge register（内部）
...
cc-bridge **不强制后台 daemon**。Scanner 通过两层触发覆盖：
...
### 7.8 cc-bridge register

内部命令，由 Hook 调用。容错设计：永不阻塞 Claude Code 启动。

### 7.9 cc-bridge export

导出会话为 markdown/text/json。流式处理，避免内存问题。

### 7.10 cc-bridge search

搜索会话标题和内容。

### 7.11 cc-bridge clean

清理无效记录。

### 7.12 cc-bridge feishu-cmd
...
| 环境变量 | 配置项 |
|----------|--------|
| `CC_BRIDGE_CONFIG_PATH` | 配置文件路径 |
| `CC_BRIDGE_REGISTRY_PATH` | Registry 路径 |
| `CC_BRIDGE_LOG_LEVEL` | 日志级别 |
| `CC_BRIDGE_TOKEN` | bridge API token |
| `CC_BRIDGE_API_URL` | bridge API 地址 |
...
| 错误码 | 说明 | 解决方式 |
|--------|------|----------|
| E001 | Registry 不存在 | 运行 `cc-bridge init` |
...
- `CCBridgeError` 提供错误码、消息、详情和修复建议
- 未知错误打印堆栈
