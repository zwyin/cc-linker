...
**日期：** 2026-06-06(v2 / v2.1 / v2.2 均为 2026-06-06 同日 review 迭代;v2.2.1 / v2.2.2 / v2.2.3 / v2.2.4 / v2.2.5 / v2.2.6 / v2.2.7 / v2.2.8 为 2026-06-07 hotfix)
**版本：** v2.2.8
**状态：** v2.2.8 已批准(Peek 改用 JSONL assistant 文本)
**作者：** Claude Code
...
| v2.2.8 | 2026-06-07 | Peek 卡 Recent output 改用 JSONL assistant 文本:<br>1) **根因** — v2.2.4 起 Peek 一直走 `claude logs <short>` + ANSI strip 包 code-block。问题在于 `claude logs` 返回的是**完整终端屏幕 buffer**(光标定位 `\x1b[<n>;<m>H`、box-drawing `│ ┌ └`、各种位置控制),`stripAnsi` 只剥 SGR 颜色/样式 escape,**不剥光标定位 / box-drawing**;飞书 monospace 字体把残留的 `│ ┌ └ \x1b[H` 等渲染成一排 □ tofu,完全无法阅读。TUI 视觉里看到的 "Spec 自检完成。**Spec 已写入并 commit：**" 实际是 JSONL 里 assistant 消息的 markdown 原文,TUI 自己把它渲染出来,跟 `claude logs` 的屏幕 buffer 是两条线<br>2) **修复 — 改数据源** — 新增 `src/agent-view/jsonl-peek.ts`:`extractRecentAssistantText(jsonlPath, maxChars)` 从后往前扫 JSONL,取第一条带 `text` block 的 assistant entry,join 多个 text block 用 `\n\n`,按段落边界截断到 1500 字符(默认),返回 markdown 原文;> 2MB 文件走 `openSync` + `readSync` 尾部 256KB(assistant 历史最新条几乎必在尾)<br>3) **fork-from-active 兜底** — bg session 经常是 `/background` fork 出来的,自己 JSONL 只有 metadata(看 92664deb 实例 = 2 行 ai-title/agent-name);`roster.json[short].dispatch.launch.sessionId` 是 parent JSONL 全路径,`roster-source.ts` 新增 `lookupResumeFromPath`,manager 三级解析链 `own JSONL → parent JSONL → claude logs`(终极兜底,仍走老 code-block 路径并加 "原始终端片段(可能含格式残留)" 警示标签)<br>4) **card 渲染分流** — `buildPeekCard` 加 `outputFormat?: 'markdown' \| 'terminal'`:markdown 直出(不包 code-block,飞书 markdown widget 渲染),terminal 退化时才包 code-block + 警示。原 `claude logs` 出错时不再发 `❌ claude logs 失败` 错误卡(因为它不再是主路径),改成显示 `(无可用输出)` 占位<br>5) **manager 解耦设计** — `AgentViewManager` 新增 static `_peekHooks = { findJsonlForShort, extractRecentAssistantText, readRoster, lookupResumeFromPath }` mutable 对象,resolvePeekContent 走它,tests swap 字段就能模拟各层命中/miss(同 v2.2.6 `_nameCacheHooks` 模式,绕 bun mock.module 跨文件不可撤销限制)<br>6) **实测对齐 TUI** — 92664deb (`Design cross-model AI review tool`) tier 2 命中,显示 `Spec 自检完成。**Spec 已写入并 commit：** - 初始 spec：\`775ce50\`...`;3a41fe73 (settled `My current model is **MiniMax-M3[1m]**.`) tier 1 命中;098639ad (settled `后台命令已完成，输出：\`\`\`done\`\`\``) tier 1 命中;视觉与 TUI 一致<br>7) **测试** — 新增 `jsonl-peek.test.ts`(11 case:string/array/混合 content、last-wins、空文件/不存在/损坏 JSON、超长截断、文件不存在等)+ `roster-source.test.ts` 加 5 lookupResumeFromPath case(resume hit / 不存在 / null roster / 非 resume mode / 非 .jsonl path)+ `manager.test.ts` 加 5 resolvePeekContent case(三层命中各一 + miss + 短路验证)+ buildPeekCard markdown 渲染 case(确认不包 code-block 不显示警示);705 pass / 0 fail |
