...
**日期：** 2026-06-06(v2 / v2.1 / v2.2 均为 2026-06-06 同日 review 迭代)
**版本：** v2.2
**状态：** v2.2 已批准(final review 后修订)
**作者：** Claude Code
...
**v2.2.1 增补:roster.json 关联 + sub-agent 过滤**
...
| `fleet` | daemon 内部任务 | 显示为 "Completed" | **过滤掉** |
| (无 roster 记录) | daemon 没跑 / session 不在 roster 中 | — | 保留(graceful) |

实现细节:每个 worker 的 `workers[shortId].dispatch.source` 会被压成 `Map<shortId, source>`,`attachRosterSources` 用 sessionId 前 8 字符查这个 Map 给 session 打 `source` 标签;`filterUserDispatched` 在它之后跑,只保留 `slash` + `unknown`。spec §5.1 详细步骤。
...
4. snapshot.filter(kind === "background")  // 关键过滤:不漏掉 kind=interactive
   ↓
5. (v2.2.1) SnapshotFetcher 内部读 `~/.claude/daemon/roster.json` → `attachRosterSources` 给每个 session 打 `source` 标签 → `filterUserDispatched` 丢弃 `source === 'spare'` 或 `'fleet'` 的 session(对应 TUI 隐藏的 sub-agent);`source === 'unknown'` 保留(daemon 没跑时不能误清空)
   ↓
