...
> **Phase 依赖关系**：
> - Phase 0（基础）必须最先完成
> - Phase 1（进程抽象）依赖 Phase 0
> - Phase 2（核心模块）依赖 Phase 0 + Phase 1
> - Phase 3（配置）和 Phase 4（Spool）独立于 Phase 1/2，可与 Phase 1/2 并行
> - Phase 5（Bot 集成）依赖 Phase 2 + Phase 3 + Phase 4
> - Phase 6（启动+CLI）依赖 Phase 5
> - Phase 7（Scanner）完全独立，可任意时机
> - Phase 8（Hook）完全独立，可任意时机
>
> **Subagent 执行建议**：
> - 串行 phase：0 → 1 → 2
> - 并行 phase：3 和 4（与 2 重叠）
> - 串行 phase：5 → 6
> - 并行 phase：7 和 8（与 6 重叠）
>
> **测试目录**：
> - `tests/unit/utils/` 已存在（`tests/unit/utils/jsonl-repair.test.ts` 等）
> - `tests/unit/queue/` 不存在，但 `tests/unit/scanner/` 存在，**新建子目录是允许的**
>
> ---
...
### Task 1.1: `ProcessInfo` 类型 + `getOwnCwd` 探测

**Files:**
- Create: `src/utils/process-info.ts`

- [ ] **Step 1: 创建模块（ESM-only，删除 require 形式）**
...
在 `src/utils/process-info.ts` 末尾添加：
...
- [ ] **Step 1: 添加函数（`getClaudeProcessesByCwd` 已过滤 cwd，直接取 [0]）**
...
- [ ] **Step 1: 添加 `ActivityEntry` 类型别名（消除内联类型重复）**

在 `export type DetectionDirection` 后加：
...
- [ ] **Step 4: 添加 `detectCliActivity`（使用 ActivityEntry）**
...
- [ ] **Step 5: 添加 `detectFeishuActivity`（使用 ActivityEntry）**
...
- [ ] **Step 6: 添加 `isSessionActive` 主入口（使用 ActivityEntry）**
...
> **重要**：活跃检测**只对 `case 'session'` 生效**。其他 case（`new_session_claim`、`new_session_creating`、`no_target`）完全跳过活跃检测——理由：
> - `new_session_claim`: session 尚未确定，检测无意义
> - `new_session_creating`: 已在创建中，detection 阻断会卡住流程
> - `no_target`: 用户没绑定 session，让 bot 引导用户
...
async createCLIBusyCard(
  ...
  // 实际 client API 调用参考 createPermissionCard（line 114 of card-updater.ts）
...
### Task 5.7: `SpoolQueue.listProcessing`（已存在，仅验证）

**Files:**
- Verify: `src/queue/spool.ts:310` 已存在 `listProcessing`
...
Expected: 输出 `310:  listProcessing(): SpoolMessage[] {`

**如果存在**：本任务无代码改动，仅记录"无需实施"。跳过到 Step 4。
...
> 注：本 plan 假设 Phase 7（Scanner 修正）实施前此方法已存在。**Task 5.6（`handleForceSendCardAction`）依赖此方法**。如果未来 SpoolQueue 重构删除了此方法，需在 Task 5.6 前补回。
...
> 注：使用 `setTimeout` Promise 包装形式（与 `src/proxy/session.ts:50` 已有 `sleep` 工具一致）。**不要**用 `import { sleep } from 'bun'`——非便携且本项目未使用。如果 `start.ts` 已有 import 形如 `function sleep(ms: number): Promise<void>`，直接复用。
...
```bash
grep -n "new FeishuBot\|sessionManager" src/cli/commands/start.ts
```

Expected: 输出中应有 `sessionManager.setActivityCache(activityCache)` 在 `new FeishuBot({...})` 之前
...
在 README.md 中"高级配置"或"可选配置"section 末尾追加：
